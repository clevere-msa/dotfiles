...
do {
    ...
    while ( $input .= <$fh> ) {
        while ($input) {
            if ( $input
                =~ s/^<($re_name)((?:\s+$re_name\s*=\s*$re_attr)*\s*)(\/?)>(.*)$/$4/s
                )
            {
                my %attr;
...
} while ( !$stdin && $opt_z && ( $fh = open_next_file( \$filename ) ) );
...
sub parseStart {
    ...
    foreach my $attr (@k) {
        ...
        #
        if (    $schemaHackMode
            and $attr =~ m/^(minOccurs|maxOccurs)$/
            and $attr{$attr} eq "1" )
        {
            next;
        }
