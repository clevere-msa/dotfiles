...
my $re = qr{
    (     (?:--)[^\n]*\n                                      # single line comments
        ...
        | /\*[\ \t\r\n\S]*?\*/                                # C style comments
        | (?:CREATE\sOR\sREPLACE)                             # keywords with spaces in them
        | (?:END\ (?:LOOP|IF))                                # keywords with spaces in them
        | \w+(?:\.\w+)*\%(?:ROWTYPE|TYPE|NOTFOUND)            # type and rowtype specifiers
        | (?:-)?\n+                                           # numbers, possibly negated
...
sub _process_rule {
    ...
    my $format = {
        ...
        pop =>
            sub { $self->{_level} = pop( @{ $self->{_level_stack} } ) || 0 },
        reset =>
...
( my $query = join "\n", map { trim($_) } <SQL> ) =~ s{\t}{    }gmsx;

my $beautifier = OraBeautify->new();
$beautifier->add_keywords(
    qw{ PIVOT    UNPIVOT  MODEL    DIMENSION
        MEASURES RULES    XMLTABLE XMLSEQUENCE
        COLUMNS  VARCHAR2
        }
);
$beautifier->add_keywords('CREATE OR REPLACE', 'END IF', 'END LOOP');

$beautifier->add_rule( 'back-token-over',  [qw(BEGIN)] );
$beautifier->add_rule( 'token-over',       ['CREATE OR REPLACE', 'LOOP'] );
$beautifier->add_rule( 'token-break-over', [qw(THEN EXCEPTION BEGIN)] );
#$beautifier->add_rule( 'back-token',       ['END', 'END IF', 'END LOOP'] );
$beautifier->add_rule( 'break-token',
    [qw(INSERT SELECT UPDATE DELETE SET FROM WHERE ORDER GROUP CURSOR AND OR)] );
$beautifier->add_rule( 'token',            [ ',', '(', ')' ] );
$beautifier->add_rule( 'token-back-break', [ '/', ';' ] );
$beautifier->query(
    $query,
    spaces      => 4,
    space       => ' ',
